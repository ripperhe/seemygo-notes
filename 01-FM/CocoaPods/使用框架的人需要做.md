...
* 常见问题:
	* ![](https://github.com/ripperhe/Seemygo-notes/blob/master/01-FM/CocoaPods/image/2_常见问题.png)
* 解决方案:
	* 删除cocoapods索引文件
...
* 须知: 可以存放到其他目录, 但必须指定项目工程的位置
	* `xcodeproj`
	* ![](https://github.com/ripperhe/Seemygo-notes/blob/master/01-FM/CocoaPods/image/2_Podfile位置.png)
* 创建命令
...
* 内部做的事情:
	...
	* 如果是之前版本
		...
		* 直接就是根据 Podfile 文件找到, 框架信息, 然后下载集成
* 常见错误
	* ![](https://github.com/ripperhe/Seemygo-notes/blob/master/01-FM/CocoaPods/image/2_常见错误.png)
	* 原因: 
	...
	* 解决方案: 
		* 指定依赖目标
			* target
		* 修改 `Podfile` 文件内容, 增加
			* ![](https://github.com/ripperhe/Seemygo-notes/blob/master/01-FM/CocoaPods/image/2_增加.png)
...
* `pod install` 和 `pod update` 区别
	* 图解
		* ![](https://github.com/ripperhe/Seemygo-notes/blob/master/01-FM/CocoaPods/image/2_图解.png)  
	* 解释
		* `installl`
	...
	* 经验: 实际项目开发过程中, 该如何选择 `install` 和 `update` 命令??
		* 如果多人开发, 首先, 上传到共享库的只有图示的四个文件!!
			* ![](https://github.com/ripperhe/Seemygo-notes/blob/master/01-FM/CocoaPods/image/2_四个文件.png)
